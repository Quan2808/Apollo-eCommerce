...
function DealsContent() {
    ...
    async function fetchData(storeId) {
        ...
        await axios.get(`http://localhost:9999/api/stores/${storeId}/products`)
            .then((res) => {
        ...
        <div className="font-shopFont">
            ...
            <div className="bg-white w-full h-[732px]">
                <div className="grid grid-cols-4 gap-2 relative px-4 py-4">
                    {categories.filter((category) => category.parentStoreCategory === null).map((category, index) => index <= 7 && (
                        <div key={category.id} className="w-[356px] h-[356px] cursor-pointer border-[1px] border-gray-200 rounded-[12px] bg-gray-200 shadow-none hover:shadow-testShadow hover:rounded-[12px] duration-200">
