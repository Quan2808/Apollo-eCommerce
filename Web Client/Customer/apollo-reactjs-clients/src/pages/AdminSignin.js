...
import { setAdminInfo } from "../features/admin/adminSlice";
...
function AdminSignin() {
  ...
  const handleChange = (event) => {
    ...
    const handleSubmit = async () => {
        setLoading(true);
        try {
            const response = await axios.post("http://localhost:9999/api/login/admin", form);

            setErrorNotify(""); // Clear any previous error notification
            setLoading(false);  // Stop loading indicator

            const accessToken = response.data.accessToken; // Extract access token
            window.localStorage.setItem("admin_token", accessToken); // Store the access token

            // Decode token and set admin info
            const adminInfo = jwt_decode(accessToken).sub; // Ensure 'sub' contains correct user info
        ...
        >
          {({ errors, handleSubmit }) => (
            ...
            >
              ...
              <Link className="w-full" to="/admincentral/register">
                <button
              ...
              border-zinc-400 active:border-yellow-800 active:shadow-amazonInput"
                >
                  Create your Apollo Admin account
                </button>
              </Link>
            </form>
          )}
